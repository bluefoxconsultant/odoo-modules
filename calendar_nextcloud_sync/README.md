...
Bidirectional calendar synchronization between Odoo 18 and Nextcloud via CalDAV + n8n webhooks.
...
```bash
# Upgrade module
docker exec my-odoo odoo -d my-database -u calendar_nextcloud_sync \
...
| Calendar Owner | Odoo user who will see synced events in their calendar | `John Doe` |
| Webhook URL | n8n webhook for Odoo &rarr; NC *(auto-filled from Settings)* | `https://n8n.example.com/webhook/bf-odoo-to-nc` |
...
|----------|-------------|
| `BF_NC_WEBHOOK_SECRET` | Secret for NC &rarr; Odoo webhook authentication |
| `BF_ODOO_WEBHOOK_SECRET` | Secret for Odoo &rarr; NC webhook authentication |
| `BF_ODOO_DB` | Odoo database name (`my-database`) |
| `BF_ODOO_USER_ID` | Odoo API user ID |
| `BF_ODOO_API_KEY` | Odoo API key |
...
curl -X POST "https://nextcloud.example.com/ocs/v2.php/apps/webhook_listeners/api/v1/webhooks" \
  ...
  -d '{
    "httpMethod": "POST",
    "uri": "https://n8n.example.com/webhook/bf-nc-to-odoo",
